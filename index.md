...
To push the change into the repository in github I used the command git add, which adds the given file nad makes it ready for you to commit the changes. So, I typed git add L < tab > .java to make it ready to be commited. Then I typed git commit -m "Fixed" to push the edit made into my github account. The command git commit pushes the the changed made into the repository and prints out the message given inside quotation. After this it gave a screen that looked like this:  
